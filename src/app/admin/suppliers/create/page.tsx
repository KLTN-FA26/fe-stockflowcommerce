...
function validateForm(form: FormState): ValidationIssue[] {
  ...
  if (!form.contactName.trim()) issues.push({ step: "contact", message: "Người liên hệ là bắt buộc." });
  if (!email) issues.push({ step: "contact", message: "Email liên hệ là bắt buộc." });
  if (email && !email.includes("@")) issues.push({ step: "contact", message: "Email liên hệ chưa đúng định dạng." });
  if (!form.contactPhone.trim()) issues.push({ step: "contact", message: "Số điện thoại là bắt buộc." });
  if (!form.street.trim()) issues.push({ step: "address", message: "Địa chỉ đường/số nhà là bắt buộc." });
  ...
  if (!form.postalCode.trim()) issues.push({ step: "address", message: "Mã bưu chính là bắt buộc." });
  if (!form.paymentTerms.trim()) issues.push({ step: "terms", message: "Điều khoản thanh toán là bắt buộc." });
  if (!Number.isFinite(leadTime) || leadTime <= 0) issues.push({ step: "terms", message: "Lead time phải lớn hơn 0 ngày." });
  if (!Number.isFinite(rating) || rating < 0 || rating > 5) issues.push({ step: "terms", message: "Rating phải nằm trong khoảng 0–5." });
...
function InfoRow({ label, value }: { label: string; value: React.ReactNode }) {
  return (
    <div className="flex items-start justify-between gap-4 border-b border-border-default py-2 last:border-b-0">
      <span className="text-xs text-ink-tertiary">{label}</span>
      <span className="text-right text-[0.8125rem] font-medium text-ink-primary">{value}</span>
    </div>
  );
}
  ...
  return (
    <>
      ...
      <div className="grid gap-4 xl:grid-cols-[260px_1fr]">
        <Card className="h-fit p-0" variant="flush">
          <div className="border-b border-border-default px-4 py-3">
            <div className="text-xs text-ink-tertiary">Trạng thái hồ sơ</div>
            <div className="mt-2">
        ...
        <div className="min-w-0 space-y-4 pb-24">
            ...
            <Card>
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <TextInput value={form.supplierId} onChange={(e) => updateForm("supplierId", e.target.value)} placeholder="SUP-NEW" />
                </div>
                <div className="space-y-1.5">
                  <FieldLabel required>Mã số thuế</FieldLabel>
                  <TextInput value={form.taxCode} onChange={(e) => updateForm("taxCode", e.target.value)} placeholder="0301234567" />
                </div>
                <div className="space-y-1.5 lg:col-span-2">
                  <FieldLabel required>Tên nhà cung cấp</FieldLabel>
                  <TextInput value={form.name} onChange={(e) => updateForm("name", e.target.value)} placeholder="Công ty TNHH..." />
                </div>
                <div className="space-y-1.5">
            ...
            <Card>
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <TextInput value={form.contactName} onChange={(e) => updateForm("contactName", e.target.value)} placeholder="Nguyễn Văn A" />
                </div>
                <div className="space-y-1.5">
                  <FieldLabel required>Số điện thoại</FieldLabel>
                  <TextInput value={form.contactPhone} onChange={(e) => updateForm("contactPhone", e.target.value)} placeholder="+84..." />
                </div>
                <div className="space-y-1.5 lg:col-span-2">
                  <FieldLabel required>Email</FieldLabel>
                  <TextInput value={form.contactEmail} onChange={(e) => updateForm("contactEmail", e.target.value)} placeholder="sales@example.com" />
                </div>
              </div>
            </Card>
          )}
            ...
            <Card>
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                <div className="space-y-1.5 lg:col-span-2">
                  ...
                  <TextInput value={form.street} onChange={(e) => updateForm("street", e.target.value)} placeholder="Số nhà, đường, KCN..." />
                </div>
                <div className="space-y-1.5">
                  <FieldLabel required>Phường / xã</FieldLabel>
                  <TextInput value={form.ward} onChange={(e) => updateForm("ward", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <FieldLabel required>Quận / huyện</FieldLabel>
                  <TextInput value={form.district} onChange={(e) => updateForm("district", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <FieldLabel required>Tỉnh / TP</FieldLabel>
                  <TextInput value={form.province} onChange={(e) => updateForm("province", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <FieldLabel required>Mã bưu chính</FieldLabel>
                  <TextInput value={form.postalCode} onChange={(e) => updateForm("postalCode", e.target.value)} />
                </div>
              </div>
            </Card>
          )}
            ...
            <Card>
              ...
              <div className="mt-4 grid gap-4 lg:grid-cols-2">
                <div className="space-y-1.5">
                  ...
                  <TextInput value={form.paymentTerms} onChange={(e) => updateForm("paymentTerms", e.target.value)} placeholder="Net 30" />
                </div>
                <div className="space-y-1.5">
                ...
                <div className="space-y-1.5">
                  ...
                  <TextInput type="number" min="1" value={form.leadTimeDays} onChange={(e) => updateForm("leadTimeDays", e.target.value)} />
                </div>
                <div className="space-y-1.5">
                  <FieldLabel required>Rating chất lượng</FieldLabel>
                  <TextInput type="number" min="0" max="5" step="0.1" value={form.rating} onChange={(e) => updateForm("rating", e.target.value)} />
                </div>
              </div>
            </Card>
          )}
          ...
          {currentStep === "review" && (
            <div className="grid gap-4 lg:grid-cols-2">
              ...
              <Card>
                <h2 className="text-sm font-semibold text-ink-primary">Checklist gửi duyệt</h2>
                <div className="mt-4 space-y-2">
                  {[
                    ...
                    <div key={label as string} className="flex items-center justify-between rounded-[var(--r-sm)] border border-border-default bg-bg-subtle px-3 py-2 text-[0.8125rem]">
                      <span className="text-ink-secondary">{label}</span>
                      <span className={ok ? "font-medium text-positive" : "font-medium text-danger"}>{ok ? "Đạt" : "Thiếu"}</span>
                    </div>
                  ))}
                </div>
              </Card>
