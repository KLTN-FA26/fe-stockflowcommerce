...
const PRODUCT_COLUMNS: ProductTableColumnKey[] = [
  "productId",
  "name",
  "category",
  "type",
...
const SKU_COLUMNS: SkuTableColumnKey[] = [
  "skuId",
  "variantLabel",
  "productId",
  "uom",
...
const DEFAULT_CONFIG: ProductsPageConfig = {
  products: {
    ...
    columnSearch: {},
    visibleColumns: PRODUCT_COLUMNS,
  },
  skus: {
    showStats: false,
    statuses: ["all"],
...
const SKU_STATUS_OPTIONS = ["all", "Active", "Blocked", "Obsolete"].map((value) => ({
  label: value === "all" ? "Tất cả" : (STATUS_LABEL_VI[value] ?? value),
...
const PRODUCT_SEARCH_FIELDS: {
  ...
  { label: "Tên sản phẩm", value: "name", getValue: (row) => row.name },
  {
    label: "Danh mục",
    value: "category",
    getValue: (row) => categoryMap.get(row.categoryId) ?? "",
  },
  { label: "Loại", value: "type", getValue: (row) => row.type },
];
const SKU_SEARCH_FIELDS: {
  label: string;
  value: SkuSearchField;
  getValue: (row: Sku) => string;
}[] = [
  ...
  { label: "Biến thể", value: "variantLabel", getValue: (row) => row.variantLabel },
  {
    label: "Sản phẩm",
    value: "product",
    getValue: (row) => productNameMap.get(row.productId) ?? row.productId,
  },
...
function computeSkuStats(list: Sku[]) {
  ...
  const lowStock = list.filter(
    (s) => s.stockAvailable <= s.reorderPoint && s.reorderPoint > 0,
  ).length;
  return [
...
function shouldFlagSku(row: Sku): boolean {
  return (
    row.status === "Blocked" || (row.stockAvailable <= row.reorderPoint && row.reorderPoint > 0)
  );
}
...
export default function ProductsPage() {
  ...
  const navigateToDetail = useCallback(
    (product: Product) => router.push(`/admin/products/${product.productId}`),
    [router],
  );
  const navigateToSkuDetail = useCallback(
    (sku: Sku) => router.push(`/admin/products/sku/${sku.skuId}`),
    [router],
  );
  ...
  const filteredProducts = useMemo(() => {
    ...
    if (categoryQuery)
      list = list.filter((product) =>
        normalize(categoryMap.get(product.categoryId) ?? "").includes(categoryQuery),
      );
    return list;
  }, [config.products]);
  ...
  const productColumns: (ColumnDef<Product> & { key: ProductTableColumnKey })[] = [
    {
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.products.columnSearch.productId ?? ""}
          label="Mã SP"
    ...
    },
    {
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.products.columnSearch.name ?? ""}
          label="Tên sản phẩm"
          placeholder="Lọc tên"
          onChange={(value) =>
            updateProductsConfig((current) => ({
    ...
    },
    {
      ...textCell<Product>(
        "category",
        "Danh mục",
        (row) => categoryMap.get(row.categoryId) ?? "—",
        {
          sortable: true,
          compare: (a, b) =>
            (categoryMap.get(a.categoryId) ?? "").localeCompare(
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.products.columnSearch.category ?? ""}
          label="Danh mục"
    ...
    },
    {
      ...
      align: "right",
      sortable: true,
      compare: (a, b) => (skuCountMap.get(a.productId) ?? 0) - (skuCountMap.get(b.productId) ?? 0),
      cell: (row) => (
  ...
  const skuColumns: (ColumnDef<Sku> & { key: SkuTableColumnKey })[] = [
    {
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.skus.columnSearch.skuId ?? ""}
          label="Mã SKU"
          placeholder="Lọc SKU"
          onChange={(value) =>
            updateSkusConfig((current) => ({
    ...
    },
    {
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.skus.columnSearch.variantLabel ?? ""}
          label="Biến thể"
    ...
    },
    {
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.skus.columnSearch.product ?? ""}
          label="Sản phẩm"
    ...
    if (activeTab === "products") {
      ...
      const summaryItems: ListSummaryItem[] = [
        { label: "Stats", value: pageConfig.showStats ? "Đang hiện" : "Đang ẩn" },
        {
          ...
          onClear: () => updateProductsConfig((current) => ({ ...current, statuses: ["all"] })),
        },
        {
          label: "Search chính",
        ...
        },
        {
          ...
          value:
            pageConfig.globalSearch.fields
              .map(
                (field) =>
                  PRODUCT_SEARCH_FIELDS.find((option) => option.value === field)?.label ?? field,
              )
              .join(", ") || "Chưa chọn",
        ...
        {
          label: "Cột hiển thị",
          value: `${visibleColumnCount}/${PRODUCT_COLUMNS.length - 1}`,
          active: hasColumnConfig,
          onClear: () =>
            updateProductsConfig((current) => ({ ...current, visibleColumns: PRODUCT_COLUMNS })),
        },
      ];
      return (
        <ListToolbar
          ...
          onClearStatuses={() =>
            updateProductsConfig((current) => ({ ...current, statuses: ["all"] }))
          }
          hasStatusFilter={hasStatusFilter}
          fieldOptions={PRODUCT_SEARCH_FIELDS}
          ...
          onSelectAllFields={() =>
            updateProductsConfig((current) => ({
              ...
              globalSearch: {
                ...current.globalSearch,
                fields: PRODUCT_SEARCH_FIELDS.map((field) => field.value),
              },
            }))
          }
          hasFieldConfig={hasFieldConfig}
          columnOptions={PRODUCT_COLUMNS.map((column) => ({
            label: PRODUCT_COLUMN_LABELS[column],
            value: column,
          }))}
          selectedColumns={pageConfig.visibleColumns}
          defaultColumns={PRODUCT_COLUMNS}
          ...
          onResetAll={() => updateProductsConfig(() => DEFAULT_CONFIG.products)}
    ...
    const summaryItems: ListSummaryItem[] = [
      { label: "Stats", value: pageConfig.showStats ? "Đang hiện" : "Đang ẩn" },
      {
        ...
        value: pageConfig.statuses
          ...
          .join(", "),
        active: hasStatusFilter,
        onClear: () => updateSkusConfig((current) => ({ ...current, statuses: ["all"] })),
      },
      {
        label: "Search chính",
        value: hasGlobalSearch ? `“${pageConfig.globalSearch.query}”` : "Chưa dùng",
      ...
      },
      {
        ...
        value:
          pageConfig.globalSearch.fields
            .map(
              (field) => SKU_SEARCH_FIELDS.find((option) => option.value === field)?.label ?? field,
            )
            .join(", ") || "Chưa chọn",
        active: hasFieldConfig,
        onClear: () =>
      ...
      {
        label: "Cột hiển thị",
        value: `${visibleColumnCount}/${SKU_COLUMNS.length - 1}`,
        active: hasColumnConfig,
        onClear: () => updateSkusConfig((current) => ({ ...current, visibleColumns: SKU_COLUMNS })),
      },
    ];
    return (
      <ListToolbar
        ...
        onClearStatuses={() => updateSkusConfig((current) => ({ ...current, statuses: ["all"] }))}
        hasStatusFilter={hasStatusFilter}
        fieldOptions={SKU_SEARCH_FIELDS}
        selectedFields={pageConfig.globalSearch.fields}
        ...
        onSelectAllFields={() =>
          updateSkusConfig((current) => ({
            ...
            globalSearch: {
              ...current.globalSearch,
              fields: SKU_SEARCH_FIELDS.map((field) => field.value),
            },
          }))
        }
        hasFieldConfig={hasFieldConfig}
        columnOptions={SKU_COLUMNS.map((column) => ({
          label: SKU_COLUMN_LABELS[column],
          value: column,
        }))}
        selectedColumns={pageConfig.visibleColumns}
        defaultColumns={SKU_COLUMNS}
        lockedColumns={["actions"]}
        visibleColumnCount={visibleColumnCount}
        onToggleColumn={(column) =>
          column !== "actions" &&
        ...
        onResetColumns={() =>
          updateSkusConfig((current) => ({ ...current, visibleColumns: SKU_COLUMNS }))
        }
        hasColumnConfig={hasColumnConfig}
        ...
        summaryItems={summaryItems}
        onResetAll={() => updateSkusConfig(() => DEFAULT_CONFIG.skus)}
        resetDisabled={!hasAnyConfig}
      />
    );
  };
    ...
    <>
        ...
        actions={
          <div className="flex items-center gap-2">
            <Button
              type="button"
              variant={
                (activeTab === "products" ? config.products.showStats : config.skus.showStats)
                  ? "secondary"
                  : "outline"
              }
              size="sm"
      ...
      {activeTab === "products" ? (
        <>
          <ListStatsPanel
            stats={productStats}
            open={config.products.showStats}
          ...
          <DataTable
            ...
            columns={productColumns.filter((column) =>
              config.products.visibleColumns.includes(column.key),
            )}
        ...
        <>
          <ListStatsPanel
            stats={skuStats}
            open={config.skus.showStats}
            gridClassName="lg:grid-cols-4 xl:grid-cols-7"
          />
          {renderToolbar()}
          <DataTable
            data={filteredSkus}
            columns={skuColumns.filter((column) => config.skus.visibleColumns.includes(column.key))}
