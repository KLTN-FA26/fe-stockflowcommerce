...
type PoTableColumnKey =
  "poNumber" | "supplier" | "warehouse" | "expectedDate" | "grandTotal" | "status" | "actions";
...
const formatMoney = (amount: number, currency: string) =>
  currency === "USD" ? formatUSD(amount) : formatVND(amount);
...
export default function PurchaseOrdersPage() {
  ...
  const { config, setConfig, updateConfig } = usePageConfig<PurchaseOrdersPageConfig>(
    STORAGE_KEY,
    DEFAULT_CONFIG,
    mergeStoredConfig,
  );
  ...
  const filtered = useMemo(() => {
    ...
    if (!config.statuses.includes("all"))
      list = list.filter((po) => config.statuses.includes(po.status));
    ...
    if (supplierQuery)
      list = list.filter((po) => normalize(supplierName(po)).includes(supplierQuery));
    const warehouseQuery = normalize(config.columnSearch.warehouse ?? "");
    if (warehouseQuery)
      list = list.filter((po) => normalize(warehouseName(po)).includes(warehouseQuery));
  ...
  const columns: (ColumnDef<PurchaseOrder> & { key: PoTableColumnKey })[] = [
    {
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.columnSearch.poNumber ?? ""}
          label="Mã PO"
          placeholder="Lọc mã PO"
          onChange={(value) => updateColumnSearch("poNumber", value)}
        />
      ),
      cell: (row) => (
        <Link
    ...
    },
    {
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.columnSearch.supplier ?? ""}
          label="Nhà cung cấp"
          placeholder="Lọc NCC"
          onChange={(value) => updateColumnSearch("supplier", value)}
        />
      ),
    },
    {
      ...
      headerFilter: (
        <ColumnFilterButton
          value={config.columnSearch.warehouse ?? ""}
          label="Kho nhận"
          placeholder="Lọc kho"
          onChange={(value) => updateColumnSearch("warehouse", value)}
        />
      ),
    },
