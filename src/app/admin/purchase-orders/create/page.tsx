...
export default function PurchaseOrderCreatePage() {
  ...
  const activeSuppliers = useMemo(() => suppliers.filter((supplier) => supplier.active), []);
  const activeWarehouses = useMemo(() => warehouses.filter((warehouse) => warehouse.active), []);
  const activeSkus = useMemo(() => skus.filter((sku) => sku.status === "Active"), []);
...
function InfoStep({
  ...
  activeWarehouses: { warehouseId: WarehouseId; name: string; code: string; active: boolean }[];
...
function ReviewStep({
  ...
  form: FormState;
  selectedSupplier?: Supplier;
  selectedWarehouse?: { warehouseId: WarehouseId; name: string; code: string };
  totals: Totals;
  currency: string;
